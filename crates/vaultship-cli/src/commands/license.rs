...
pub async fn run(action: LicenseCommands) -> anyhow::Result<()> {
    match action {
        ...
        LicenseCommands::Validate {
            license_file,
            public_key,
        } => validate(&license_file, &public_key),
...
fn create(
    ...
    let fp = if hardware_bind {
        if let Some(path) = fingerprint {
            Some(serde_json::from_str::<HardwareFingerprint>(
...
fn fingerprint() -> anyhow::Result<()> {
    println!(
        "{}",
        serde_json::to_string_pretty(&HardwareFingerprint::collect()?)?
    );
    Ok(())
}
